...
export function BudgetPanel({account,busy,onSubmit}:{account:BudgetAccount|null;busy:boolean;onSubmit:(action:string,body:unknown)=>void}) {
 ...
 return <section aria-label="작업 예산" className="budget-panel">
  ...
  {account && <p>입력 {account.used.inputTokens.toLocaleString()} / {account.policy.total.inputTokens.toLocaleString()} · 출력 {account.used.outputTokens.toLocaleString()} / {account.policy.total.outputTokens.toLocaleString()} 토큰<br/>집계 시작: {new Date(account.startedAt).toLocaleString()}{account.pause && ` · ${account.pause.reason.replaceAll("inputTokens","입력 토큰").replaceAll("outputTokens","출력 토큰").replaceAll("durationMs","실행 시간")}`}</p>}
  ...
  {editing && <form onSubmit={submit} key={account?.version??0}>
   <p>검증된 기본값이 없으면 상한을 직접 입력하세요. 입력 토큰에는 캐시 입력이 포함됩니다.</p>
   {(["execution","total"] as const).map(scope=><fieldset key={scope}><legend>{scope==="execution"?"실행당 상한":"토픽 누적 상한"}</legend>
