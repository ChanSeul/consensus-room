...
리뷰에서 사용자에게 물은 질문은 답변이 확인될 때까지 남습니다. 보류나 다른 작업 승인, 자동 구현 재개로 질문이 사라지지 않습니다. 답변 후 재시도하면 기존 Codex 세션에서 해당 질문만 확인하며, 확인된 질문만 해소합니다. 같은 답변은 한 번만 확인하고 실패해도 자동으로 반복하지 않습니다. 이 확인에도 기존 실행·리뷰 예산이 적용됩니다.
