...
[작업 흐름](#작업-흐름) · [시스템 구조](#시스템-구조) · [Claude 중재 세션](#claude-중재-세션) · [설치와 실행](#설치와-실행)
...
- macOS, **Node.js 26 이상**, Git이 필요합니다.
- 실제 모델을 호출하려면 Claude CLI와 Codex CLI가 설치되어 있고 로그인되어 있어야 합니다.
- 현재 구현은 개인 개발 환경에 맞춰져 있습니다. 작업 저장소, 공용 메모리와 스킬 디렉터리는 [설정 코드](src/server/config.ts)의 기본 경로를 사용합니다. 다른 환경에서는 이 경로를 먼저 조정해야 하며, 새 주제 요청에 저장소 경로를 넣는 방식으로 바꿀 수 없습니다.
- 중재 세션 없이 웹 UI로도 주제를 만들고 계획·승인·구현·리뷰를 진행할 수 있습니다.
...
서버가 출력하는 접속 주소를 같은 Mac의 브라우저에서 엽니다. 기본 주소는 `127.0.0.1:4317`이며, 접속 토큰은 확인 후 `HttpOnly` 쿠키로 전환됩니다.
...
| 환경 변수 | 용도 |
| --- | --- |
| `CONSENSUS_ROOM_DATA_DIR` | DB·주제·worktree 저장 위치. 직접 지정할 때는 먼저 디렉터리를 만들어야 합니다. |
| `CONSENSUS_ROOM_PORT` | 로컬 서버 포트. 기본값은 `4317`입니다. |
| `CONSENSUS_ROOM_TOKEN` | 접속 토큰 지정. 생략하면 실행 시 생성합니다. |
| `CONSENSUS_ROOM_CLAUDE_MODEL`, `CONSENSUS_ROOM_CLAUDE_EFFORT` | 새 주제의 Claude 기본 설정. |
| `CONSENSUS_ROOM_CODEX_MODEL`, `CONSENSUS_ROOM_CODEX_EFFORT` | 새 주제의 Codex 기본 설정. |
| `CONSENSUS_ROOM_CODEX_CONCURRENCY` | 주제 간 Codex 동시 실행 상한. 기본값은 `2`입니다. |
| `CONSENSUS_ROOM_FIGMA_MCP_URL` | Claude에 연결할 Figma 읽기 도구의 주소. 빈 문자열로 끌 수 있습니다. |

모델과 추론 강도는 주제별로 저장합니다. 환경 변수는 새 주제의 기본값이며, 기존 주제 설정을 덮어쓰지 않습니다. 화면에서 바꾼 설정은 다음 해당 역할 호출부터 적용됩니다. Claude의 구현 단계 설정은 계획 단계와 따로 지정할 수 있습니다.

기본 데이터 위치는 `~/Library/Application Support/ConsensusRoom`입니다. 상태·이벤트·실행 기록은 SQLite에, 계획과 보고서는 주제별 파일에 저장합니다.
...
npm run build:web  # 웹 정적 파일 빌드
npm run check      # 테스트 + TypeScript 검사 + 웹 빌드
```
...
이 저장소는 개발 저장소에서 검토한 파일만 옮긴 공개본입니다. 개인 운영 문서와 개발 이력은 포함하지 않습니다. 설정 코드의 예제 경로를 실행 환경에 맞게 수정해 사용합니다.
