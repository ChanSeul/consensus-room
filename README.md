...
중재자는 승인된 작업의 실행 순서와 동등한 방법을 직접 판단합니다. 기존 결정으로 해결되지 않은 범위·계약 변경이나
검증 기준 완화만 사용자에게 묻고, 답에 의존하지 않는 준비를 먼저 끝냅니다. 현재 완료 조건에 영향을 주지 않는
기존 결함과 주변 개선은 후속 목록에 보존하며, 그 처분 선택만으로 인도를 멈추지 않습니다. 필수 검증 실패나
미답 승인 요청은 계속 차단합니다. 이 기준은 중재자 운영 지침과 구현·리뷰 프롬프트에 반영되며,
모델이 항상 올바르게 분류한다고 보장하는 자동 승인 기능은 아닙니다.
...
ON이어도 새로운 범위나 중요한 계약 변경, 되돌리기 어려운 결정, 사용자만 할 수 있는 인증은 사용자에게 넘깁니다. 현재 완료 조건을 막지 않는 후속 항목은 보존하고 전달을 계속하며, 처분 선택을 선행 조건으로 만들지 않습니다.
...
Slack·Jira·Figma 원문을 주제에 연결하면 변경을 확인하고 같은 세션에는 바뀐 내용만 전달할 수 있다.
디자인 PNG와 원문은 내용 해시로 공유하며, 원문이 바뀌면 현재 계획에서 변경 영향을 확인하기 전까지 전달을 막는다.
연결 방식과 호스트 수집 절차는 [외부 근거 안내](docs/external-evidence.md)에 설명되어 있다.
...
| 사용자 결정을 기다린 뒤 같은 작업을 반복 | 저장된 계획·수정·리뷰 결과가 현재 조건과 맞으면 재사용합니다. 리뷰는 완료된 검토에 한하며, 검토 이후 새 증거·사용자 메모가 있으면 다시 검토합니다. |
...
토픽의 예산 설정에서 실행당 상한과 토픽 누적 상한을 지정합니다. 입력·출력 토큰과 실행 시간을 각각 검사하며, 자동 교정과 재시도도 같은 누적 원장을 사용합니다. 한도에 닿으면 새 모델 호출을 막고 현재 응답에는 최대 60초의 유예를 준 뒤 종료 절차를 시작합니다. 관측 지연과 유예 중에는 설정한 상한을 넘을 수 있습니다. 저장된 결과·작업 파일·세션은 보존하며, 예산을 명시적으로 늘려야 계속할 수 있습니다.

검증된 사용량 표본이 없는 환경에는 임의의 기본값을 넣지 않습니다. 상한을 직접 입력하거나, 독립적으로 확인한 정상 실행 표본으로 `scripts/calibrate-budgets.ts`를 실행해 산정한 값을 사용합니다. 화면의 합계에는 관측된 사용량만 포함되며, 집계 시작 전 사용량을 소급해 0으로 확정하지 않습니다.

긴 작업은 왼쪽의 **긴 작업을 단계로 나누기**에서 전체 목표, 공통 계약, 단계별 목표·완료 조건을 입력합니다. 각 단계는 별도 토픽에서 계획을 합의하고 사용자가 승인한 뒤 구현합니다. 앞 단계의 검토와 푸시가 끝나야 그 전달 커밋을 기준으로 다음 단계를 열 수 있습니다. 마지막 단계는 전체 통합 검증입니다. 모든 단계의 소비는 작업 묶음에도 누적되며, 단계 전환으로 예산을 초기화하지 않습니다.

공통 계약을 변경하면 미완료 단계의 기존 승인과 세션을 취소하고 다시 계획합니다. 완료한 단계의 목표와 생성 시 정한 단계 순서는 유지합니다. 운영 중 서버에는 재배포 후 반영됩니다.

### 재작성과 리뷰 호출 횟수

새 토픽은 계획 재작성 3회, 계획 검토 3회, 구현 리뷰 3회를 각각 허용합니다. 재작성은 최초 계획 1회만 제외하고 재계획·개정·자동 교정·실패한 호출을 함께 셉니다. 계획 검토는 감사와 종결 검사, 구현 리뷰는 코드 리뷰와 최종 리뷰를 합산하며 첫 호출부터 셉니다. ACK와 구현 호출은 이 횟수에서 제외합니다.

마지막 허용 호출의 결과는 저장하고 처리합니다. 다음 호출이 필요하면 계획·승인·세션·작업 파일과 재개 단계를 보존한 채 멈춥니다. 화면에서 해당 종류의 **1회 추가 승인 후 재개**를 선택해야 한 번 더 호출할 수 있습니다. 토큰·시간 예산도 별도로 남아 있어야 하며, 일반 재시도·429 자동 재시도·범위 변경·서버 재시작으로 횟수가 초기화되지 않습니다. 각 단계 토픽은 별도로 집계합니다.

기존 토픽은 확인 가능한 호출만 복구하고 기록이 불완전하다고 표시합니다. 과거 호출이 없었다고 가정해 새 3회를 주지 않으며, 다음 작성·리뷰 호출 전에 해당 종류를 1회 추가 승인해야 합니다.

리뷰에서 사용자에게 물은 질문은 답변이 확인될 때까지 남습니다. 보류나 다른 작업 승인, 자동 구현 재개로 질문이 사라지지 않습니다. 답변 후 재시도하면 기존 Codex 세션에서 해당 질문만 확인하며, 확인된 질문만 해소합니다. 같은 답변은 한 번만 확인하고 실패해도 자동으로 반복하지 않습니다. 이 확인에도 기존 실행·리뷰 예산이 적용됩니다. 이후 새 결정이 오면 이전 답변의 취소·변경도 함께 확인합니다. 전달 대기나 로컬 커밋 이후에도 확인 전에는 커밋·푸시를 막고, 계획·코드·검토 증거가 그대로면 코드 리뷰와 기존 커밋을 보존해 이어갑니다.
